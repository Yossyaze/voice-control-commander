...
interface SidebarProps {
    ...
    onReorderStrokes: (commandId: string, oldIndex: number, newIndex: number) => void;
}
    ...
    return (
        ...
        >
             <div className="flex items-center flex-1 min-w-0">
                ...
                <div className={`w-2 h-2 rounded-full mr-2.5 flex-shrink-0 ${isSelected ? 'bg-blue-500 ring-2 ring-blue-200' : 'bg-gray-400'}`}></div>
...
const SortableCommandItem = ({
    ...
    return (
         <div ref={setNodeRef} style={style} className={`rounded-md border shadow-sm transition-all overflow-hidden mb-2 ${isSelected ? 'border-blue-400 bg-white ring-1 ring-blue-100' : 'border-gray-300 bg-white hover:border-gray-300'}`}>
            {/* Command Header */}
            <div
            ...
            >
                ...
                <div className="flex items-center mb-1.5 min-w-0">
                    ...
                    {/* Color Indicator */}
                    <div className="w-2.5 h-2.5 rounded-full mr-2.5 ring-1 ring-black/10 flex-shrink-0 shadow-sm" style={{ backgroundColor: command.color }} />
                    ...
                    {editingId === command.id ? (
                        ...
                        <span
                            ...
                            onDoubleClick={(e) => { e.stopPropagation(); onStartEditing(command.id, command.name); }}
                            title={command.name}
                        >
                            {command.name}
                        </span>
            ...
            {isSelected && (
                <div className="bg-white border-t border-blue-100 pl-4 pr-3 py-2">
                    {/* Nested Dnd Context for Strokes */}
                     <DndContext sensors={strokeSensors} collisionDetection={closestCenter} onDragEnd={handleStrokeDragEnd}>
                        <SortableContext items={command.strokes.map((_, i) => `${command.id}-stroke-${i}`)} strategy={verticalListSortingStrategy}>
                                ...
                                <React.Fragment key={`${command.id}-stroke-${index}`}>
                                    ...
                                    */} 
                                    {index > 0 && (
                                        ...
                                        >
                                            ...
                                            <span className={`text-[10px] ml-2 ${selectedStrokeIndex === index - 1 && selectionType === 'wait' ? 'text-blue-700 font-bold' : 'text-gray-500'}`}>
                                                待機 {(command.strokeMetadata?.[index - 1]?.waitAfter ?? command.waitDuration ?? 0.2)}秒
                                            </span>
                                        </div>
                                    )}
                                    ...
                                    <SortableStrokeItem 
                                        ...
                                        isSelected={selectedStrokeIndex === index && selectionType === 'stroke'}
                                        onSelect={() => { onSelectStroke(index); onSelectType?.('stroke'); }}
                                        onDelete={() => {
                    ...
                    <div className="pt-3 pb-1 flex justify-center space-x-2 border-t border-dashed border-gray-200 mt-2">
                            <button
                                ...
                                onUpdateCommand({ ...command, strokes: [...command.strokes, newStroke] });
                            }}
                        ...
                        <button
                                ...
                                onUpdateCommand({ ...command, strokes: [...command.strokes, newTap] });
                            }}
                            ...
                            <ul className="space-y-0">
                                {commands.map((cmd) => (
                                    <SortableCommandItem
                                        ...
                                        onReorderStrokes={(oldIndex, newIndex) => onReorderStrokes(cmd.id, oldIndex, newIndex)}
                                    />
                                ))}
                            </ul>
                        </SortableContext>
